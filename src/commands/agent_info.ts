...
const RECOVERY: Record<ErrorCode, string> = {
  ...
  MODEL_NOT_FOUND: 'Run `nanaban agent-info` and pick from the `models` array. Canonical ids: gpt-image-2, nb2, nb-lite. Family aliases also work and always resolve to the latest model: `gpt`, `nano banana`, `full`, `lite`, `pro`.',
  ...
  const manifest = {
    ...
    verified: {
      ...
      findings: [
        ...
        'Nano Banana Pro (gemini-3-pro-image) is Gemini 3, a generation behind nb2 (Gemini 3.1), and was removed in v7 — nanaban never ships a superseded model.',
      ],
    },
    pricing_note: 'Costs live on routes, not models: est_cost_per_image_usd is the 1K estimate and Gemini pricing scales with output size (nb2 is ~$0.15 at 4K). Routes with billing: subscription_quota report cost_usd: 0 because they debit the ChatGPT subscription, not a metered balance. OpenRouter routes report actual metered cost in the envelope when available.',
    auth_resolution: {
    ...
    commands: [
      {
        ...
        flags: [
          ...
          { name: '--model', type: 'string', default: 'auto (see auth_resolution.model_auto_selection)', description: 'gpt-image-2 | nb2 | nb-lite. Names are matched ignoring case, spaces and punctuation, and family names always resolve to the CURRENT model, never a superseded one: `gpt`/`gpt image` → GPT Image 2, `nb`/`nano banana`/`full` → Nano Banana 2, `lite` → Nano Banana 2 Lite. `pro` → Nano Banana 2, because Nano Banana Pro is Gemini 3 Pro Image, a generation behind, and was removed.' },
